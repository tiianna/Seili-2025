...
In this exercise, we will work with the obtained tables for the significant transcription factors. Find them [here]() in case you want to start with the original data.

### 2.1 Protein network retrieval

Close the current session in Cytoscape from the menu **File → Close**. 

In the following, we will dig deeper into the biological context of one of the four outputs from Lisa. It is your choice whiche one to choose. However the downstream analysis about the 
protein structures will be based transcription regulators that associate with on PCB118-repressed genes: _EDCMET_symbols_PCB118_dn_p001_logFC05_top500-by-pvalue.txt_chipseq_cauchy_combine_dedup.csv_
Take on of the files and open it. You can choose how many of the most significant regulators you want to include. These can be all with a p-value below e.g. 0.05 or just the top 50 (particularly when there are more than 100).
